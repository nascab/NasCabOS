...
const getQueryFileName = url => {
    try {
        ...
        for (const candidate of candidates) {
            ...
            const fileName = getFileNameFromPath(decoded)
            if (fileName) return fileName
        }
        return getFileNameFromPath(decodeMaybe(parsed.pathname))
    } catch {
        return ''
    }
}

const getHeaderFileName = res => {
    const disposition = res.headers.get('content-disposition') ?? ''
    const encodedMatch = disposition.match(/filename\*\s*=\s*[^']*''([^;]+)/i)
    if (encodedMatch?.[1]) return getFileNameFromPath(decodeMaybe(encodedMatch[1]))
    const match = disposition.match(/filename\s*=\s*"([^"]+)"|filename\s*=\s*([^;]+)/i)
    const value = match?.[1] ?? match?.[2]
    return getFileNameFromPath(decodeMaybe(value))
}
...
const fetchFile = async (url, fallbackName) => {
    const res = await fetch(url)
    if (!res.ok) throw new ResponseError(
        `${res.status} ${res.statusText}`, { cause: res })
    const blob = await res.blob()
    const fileName = decodeMaybe(fallbackName)
        || getHeaderFileName(res)
        || getQueryFileName(res.url)
        || getQueryFileName(url)
        || 'book'
    const type = blob.type || inferMimeType(fileName)
    return new File([blob], fileName, type ? { type } : undefined)
}

const isText = ({ name, type }) => {
...
const stripFileExtension = name => {
    const fileName = getFileNameFromPath(name)
    return fileName.replace(/\.[^.]+$/, '') || fileName || 'Untitled'
}

const makeTextBook = async file => {
...
<pre>${escapeHTML(text)}</pre>
    ...
    return {
        ...
        sections: [{
            id: 0,
            load: async () => url,
            createDocument: () => new DOMParser()
                .parseFromString(html, 'application/xhtml+xml'),
            size: Math.max(file.size, text.length, 1),
        }],
...
export const makeBook = async file => {
    if (typeof file === 'string') file = await fetchFile(file,
        pageParams.get('filename') ?? pageParams.get('fileName'))
    let book
    if (file.isDirectory) {
