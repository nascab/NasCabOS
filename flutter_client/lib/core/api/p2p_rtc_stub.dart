...
class P2pRtcClient {
  ...
  /// 大流量通道发送 ACK 等控制帧前等待本 DC 出站缓冲回落，避免占满 SCTP 导致 api 控制帧发不出去。
  ...
  Future<void> _yieldBulkBuffersBeforeHighPriorityControlSend() async {
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      for (final e in _channels.entries) {
        if (!_isBulkChannel(e.key)) continue;
        final d = e.value.dc;
        if (d.state != RTCDataChannelState.RTCDataChannelOpen) continue;
        if ((d.bufferedAmount ?? 0) > perDcLimit) {
          congested = true;
          break;
        }
      }
  ...
  void _p2pDebugLog(String message) {
  ...
  void _writeBytes(BytesBuilder b, Uint8List payload) {
    _writeVarint(b, payload.length);
    if (payload.isNotEmpty) b.add(payload);
  }
  ...
  void _sendControlMessage(_RtcChannelState st, Map<String, dynamic> msg) {
    ...
    if (dc.state != RTCDataChannelState.RTCDataChannelOpen) {
      throw StateError('p2p_dc_not_open');
    }
    ...
    st.txBinaryPackets += 1;
    st.txBinaryBytes += bin.length;
    st.lastTxAtMs = DateTime.now().millisecondsSinceEpoch;
    dc.send(RTCDataChannelMessage.fromBinary(bin));
  }
  ...
  Future<void> start({List<P2pRtcChannel>? channels}) async {
    ...
    final config = <String, dynamic>{
      ...
      // 预热候选池：在 createOffer 前预先 gather 候选，减少 relay vs IPv6 的竞速随机性
      'iceCandidatePoolSize': 2,
    };
    final policy = (iceTransportPolicy ?? '').trim();
    final skipRelayCandidateDelay = policy.toLowerCase() == 'relay';
    if (policy.isNotEmpty) {
      config['iceTransportPolicy'] = policy;
    }
    final pc = await createPeerConnection(config, <String, dynamic>{});
    _pc = pc;

    pc.onIceCandidate = (c) {
      final candidate = c.candidate;
      if (candidate == null || candidate.trim().isEmpty) {
        debugPrint('[P2P-ICE] 候选收集完成 sid=$sid');
        return;
      }
      final parts = candidate.split(' ');
      final addr = parts.length > 4 ? parts[4] : '?';
      final port = parts.length > 5 ? parts[5] : '?';
      final proto = parts.length > 2 ? parts[2] : '?';
      final typIdx = parts.indexOf('typ');
      final typ = typIdx >= 0 && typIdx + 1 < parts.length
          ? parts[typIdx + 1]
          : '?';
      final isIPv6 = addr.contains(':');
      debugPrint(
        '[P2P-ICE] ${isIPv6 ? "[IPv6]" : "[IPv4]"} $proto $typ $addr:$port | sid=$sid',
      );
      ...
      if (typ == 'relay' && !skipRelayCandidateDelay) {
        ...
        Timer(_relayCandidateDelay, () {
          ...
          if (cs == RTCPeerConnectionState.RTCPeerConnectionStateConnected) {
            debugPrint(
              '[P2P-ICE] 跳过中继候选（已直连）$proto $typ $addr:$port | sid=$sid',
            );
            return;
          }
    ...
    pc.onConnectionState = (state) {
      debugPrint('[P2P-ICE] 连接状态变化 -> $state | sid=$sid');
      if (state == RTCPeerConnectionState.RTCPeerConnectionStateConnected) {
        Future.delayed(const Duration(milliseconds: 800), () async {
          try {
            final stats = await pc.getStats();
            // 找所有 succeeded 对，区分是否 nominated（实际使用的路径）
            final succeededPairs = stats
                .where(
                  (s) =>
                      s.type == 'candidate-pair' &&
                      s.values['state'] == 'succeeded',
                )
                .toList();
            StatsReport? nominatedPair;
            for (final s in succeededPairs) {
              final nominated = s.values['nominated'];
              if (nominated == true || nominated == 'true' || nominated == 1) {
                nominatedPair = s;
                break;
              }
            }
            // 没有 nominated 标记时取字节数最多的（最可能是实际传输路径）
            nominatedPair ??= succeededPairs.isEmpty
                ? null
                : succeededPairs.reduce((a, b) {
                    final aBytes =
                        (a.values['bytesSent'] as num? ?? 0) +
                        (a.values['bytesReceived'] as num? ?? 0);
                    final bBytes =
                        (b.values['bytesSent'] as num? ?? 0) +
                        (b.values['bytesReceived'] as num? ?? 0);
                    return aBytes >= bBytes ? a : b;
                  });

            for (final s in succeededPairs) {
              final localId = s.values['localCandidateId'];
              final remoteId = s.values['remoteCandidateId'];
              final local = stats.firstWhere(
                (e) => e.id == localId,
                orElse: () => s,
              );
              final remote = stats.firstWhere(
                (e) => e.id == remoteId,
                orElse: () => s,
              );
              final localAddr =
                  local.values['address'] ?? local.values['ip'] ?? '?';
              final localPort = local.values['port'] ?? '?';
              final localType = local.values['candidateType'] ?? '?';
              final localProto = local.values['protocol'] ?? '?';
              final remoteAddr =
                  remote.values['address'] ?? remote.values['ip'] ?? '?';
              final remotePort = remote.values['port'] ?? '?';
              final remoteType = remote.values['candidateType'] ?? '?';
              final isIPv6Local = localAddr.toString().contains(':');
              final isIPv6Remote = remoteAddr.toString().contains(':');
              final isNominated = s == nominatedPair;
              final bytesSent = s.values['bytesSent'] ?? 0;
              final bytesRecv = s.values['bytesReceived'] ?? 0;
              debugPrint(
                '[P2P-ICE] ${isNominated ? "★ [已选定]" : "  [succeeded]"} sid=$sid'
                ' 本地:${isIPv6Local ? "IPv6" : "IPv4"} $localProto $localType $localAddr:$localPort'
                ' 远端:${isIPv6Remote ? "IPv6" : "IPv4"} $remoteType $remoteAddr:$remotePort'
                ' tx=${bytesSent}B rx=${bytesRecv}B',
              );
            }
          } catch (e) {
            debugPrint('[P2P-ICE] getStats 失败: $e');
          }
        });
      }
    ...
    Future<void> attachChannel(P2pRtcChannel c) async {
      final prefix = _prefixForChannel(c);
      final ready = Completer<void>();
      final dc = await pc.createDataChannel(prefix, RTCDataChannelInit());
      ...
      dc.onDataChannelState = (state) {
        ...
        if (state == RTCDataChannelState.RTCDataChannelClosed) {
          ...
          final pending = List<Completer<P2pApiResponse>>.from(
            st.pending.values,
          );
          ...
          final tunnels = List<_P2pRtcWebSocketChannel>.from(
            st.wsTunnels.values,
          );
      ...
      dc.onMessage = (m) {
        st.rxQueue = st.rxQueue
            .catchError((_) {})
            .then((_) => _handleIncomingDcMessage(st, m));
      };
    }
  ...
  Future<Map<String, String>> getTransportStats() async {
    ...
    try {
      ...
      // 优先选 nominated 的候选对（实际使用的路径），其次取字节最多的
      final succeededPairs = stats
          .where(
            (s) =>
                s.type == 'candidate-pair' && s.values['state'] == 'succeeded',
          )
          .toList();
      ...
      if (activePair == null && succeededPairs.isNotEmpty) {
        activePair = succeededPairs.reduce((a, b) {
          final aBytes =
              (a.values['bytesSent'] as num? ?? 0) +
              (a.values['bytesReceived'] as num? ?? 0);
          final bBytes =
      ...
      for (final s in stats) {
        ...
        if (s.type == 'candidate-pair' && s.values['state'] == 'succeeded') {
          ...
          out['protocol'] = protocol.toString();
          out['type'] = type.toString();

          final rtt = s.values['currentRoundTripTime'];
          if (rtt != null) {
            final rttNum = rtt is num ? rtt : num.tryParse(rtt.toString());
            if (rttNum != null) {
              out['rttMs'] = (rttNum * 1000).round().toString();
            }
          }

          final bytesSent = s.values['bytesSent'];
          if (bytesSent != null) out['bytesSent'] = bytesSent.toString();
          final bytesReceived = s.values['bytesReceived'];
          if (bytesReceived != null) {
            out['bytesReceived'] = bytesReceived.toString();
          }
          final packetsSent = s.values['packetsSent'];
          if (packetsSent != null) out['packetsSent'] = packetsSent.toString();
          final packetsReceived = s.values['packetsReceived'];
          if (packetsReceived != null) {
            out['packetsReceived'] = packetsReceived.toString();
          }
          final outBitrate = s.values['availableOutgoingBitrate'];
          if (outBitrate != null) {
            out['availableOutgoingBitrate'] = outBitrate.toString();
          }
          final inBitrate = s.values['availableIncomingBitrate'];
          if (inBitrate != null) {
            out['availableIncomingBitrate'] = inBitrate.toString();
          }

          final api = _channels[P2pRtcChannel.api];
          final file = _channels[P2pRtcChannel.file];
          final upload = _channels[P2pRtcChannel.upload];
          final download = _channels[P2pRtcChannel.download];
          final video = _channels[P2pRtcChannel.video];
          if (api != null) {
            out['apiDcBuffered'] = (api.dc.bufferedAmount ?? 0).toString();
            out['apiTxPackets'] = api.txPackets.toString();
            out['apiTxBytes'] = api.txBytes.toString();
            out['apiRxPackets'] = api.rxPackets.toString();
            out['apiRxBytes'] = api.rxBytes.toString();
            out['apiRxBinaryPackets'] = api.rxBinaryPackets.toString();
            out['apiRxTextPackets'] = api.rxTextPackets.toString();
            out['apiTxBinaryPackets'] = api.txBinaryPackets.toString();
            out['apiTxTextPackets'] = api.txTextPackets.toString();
            if (api.lastRxAtMs > 0) {
              out['apiRxIdleMs'] = (nowMs - api.lastRxAtMs).toString();
            }
            if (api.lastTxAtMs > 0) {
              out['apiTxIdleMs'] = (nowMs - api.lastTxAtMs).toString();
            }
          }
          if (file != null) {
            out['fileDcBuffered'] = (file.dc.bufferedAmount ?? 0).toString();
            out['fileTxPackets'] = file.txPackets.toString();
            out['fileTxBytes'] = file.txBytes.toString();
            out['fileRxPackets'] = file.rxPackets.toString();
            out['fileRxBytes'] = file.rxBytes.toString();
            out['fileRxBinaryPackets'] = file.rxBinaryPackets.toString();
            out['fileRxTextPackets'] = file.rxTextPackets.toString();
            out['fileTxBinaryPackets'] = file.txBinaryPackets.toString();
            out['fileTxTextPackets'] = file.txTextPackets.toString();
            if (file.lastRxAtMs > 0) {
              out['fileRxIdleMs'] = (nowMs - file.lastRxAtMs).toString();
            }
            if (file.lastTxAtMs > 0) {
              out['fileTxIdleMs'] = (nowMs - file.lastTxAtMs).toString();
            }
          }
          if (upload != null) {
            out['uploadDcBuffered'] = (upload.dc.bufferedAmount ?? 0)
                .toString();
            out['uploadTxPackets'] = upload.txPackets.toString();
            out['uploadTxBytes'] = upload.txBytes.toString();
            out['uploadRxPackets'] = upload.rxPackets.toString();
            out['uploadRxBytes'] = upload.rxBytes.toString();
            out['uploadRxBinaryPackets'] = upload.rxBinaryPackets.toString();
            out['uploadRxTextPackets'] = upload.rxTextPackets.toString();
            out['uploadTxBinaryPackets'] = upload.txBinaryPackets.toString();
            out['uploadTxTextPackets'] = upload.txTextPackets.toString();
            if (upload.lastRxAtMs > 0) {
              out['uploadRxIdleMs'] = (nowMs - upload.lastRxAtMs).toString();
            }
            if (upload.lastTxAtMs > 0) {
              out['uploadTxIdleMs'] = (nowMs - upload.lastTxAtMs).toString();
            }
          }
          if (download != null) {
            out['downloadDcBuffered'] = (download.dc.bufferedAmount ?? 0)
                .toString();
            out['downloadTxPackets'] = download.txPackets.toString();
            out['downloadTxBytes'] = download.txBytes.toString();
            out['downloadRxPackets'] = download.rxPackets.toString();
            out['downloadRxBytes'] = download.rxBytes.toString();
            out['downloadRxBinaryPackets'] = download.rxBinaryPackets
                .toString();
            out['downloadRxTextPackets'] = download.rxTextPackets.toString();
            out['downloadTxBinaryPackets'] = download.txBinaryPackets
                .toString();
            out['downloadTxTextPackets'] = download.txTextPackets.toString();
            if (download.lastRxAtMs > 0) {
              out['downloadRxIdleMs'] = (nowMs - download.lastRxAtMs)
                  .toString();
            }
            if (download.lastTxAtMs > 0) {
              out['downloadTxIdleMs'] = (nowMs - download.lastTxAtMs)
                  .toString();
            }
          }
          if (video != null) {
            out['videoDcBuffered'] = (video.dc.bufferedAmount ?? 0).toString();
            out['videoTxPackets'] = video.txPackets.toString();
            out['videoTxBytes'] = video.txBytes.toString();
            out['videoRxPackets'] = video.rxPackets.toString();
            out['videoRxBytes'] = video.rxBytes.toString();
            out['videoRxBinaryPackets'] = video.rxBinaryPackets.toString();
            out['videoRxTextPackets'] = video.rxTextPackets.toString();
            out['videoTxBinaryPackets'] = video.txBinaryPackets.toString();
            out['videoTxTextPackets'] = video.txTextPackets.toString();
            if (video.lastRxAtMs > 0) {
              out['videoRxIdleMs'] = (nowMs - video.lastRxAtMs).toString();
            }
            if (video.lastTxAtMs > 0) {
              out['videoTxIdleMs'] = (nowMs - video.lastTxAtMs).toString();
            }
          }
  ...
  /// 下载专用数据通道是否 open（与主连接一并协商时可直接复用，无需第二套 P2P 会话）。
  ...
  /// 上传专用数据通道是否 open（与主连接一并协商时可直接复用）。
  ...
  /// 视频专用数据通道是否 open（open 时可复用主连接，避免单独创建 video link）。
  ...
  Future<void> _handleIncomingDcMessage(
    _RtcChannelState st,
    RTCDataChannelMessage m,
  ) async {
    if (!m.isBinary) return;
    final bytes = m.binary;
    if (bytes.isNotEmpty && (bytes[0] == 0x01 || bytes[0] == 0x02)) {
      final nowMs = DateTime.now().millisecondsSinceEpoch;
      st.rxPackets += 1;
      st.rxBytes += bytes.length;
      st.rxBinaryPackets += 1;
      st.rxBinaryBytes += bytes.length;
      st.lastRxAtMs = nowMs;
  ...
  WebSocketChannel openWebSocketChannel({
    ...
    () async {
      try {
        ...
        if (st.dc.state != RTCDataChannelState.RTCDataChannelOpen) {
  ...
  Future<P2pApiResponse> sendRequest({
    ...
    Future<void> waitDrain() async {
      ...
      final startedAt = DateTime.now();
      while ((dc.bufferedAmount ?? 0) > limit) {
    ...
    try {
      ...
      if (rawBody.isNotEmpty) {
        ...
        while (offset < rawBody.length) {
          ...
          st.txPackets += 1;
          st.txBytes += packet.length;
          st.txBinaryPackets += 1;
          st.txBinaryBytes += packet.length;
          st.lastTxAtMs = DateTime.now().millisecondsSinceEpoch;
  ...
  Future<P2pApiStreamResponse> sendRequestStream({
    ...
    try {
      ...
      _failP2pStreamBody(controller, Exception('p2p_stream_send_failed'));
      rethrow;
    }
  ...
  Future<void> close() async {
    ...
    final channels = Map<P2pRtcChannel, _RtcChannelState>.from(_channels);
    _channels.clear();
    ...
    for (final st in channels.values) {
      ...
      for (final c in pending) {
        if (!c.isCompleted) c.completeError(Exception('p2p_closed'));
      }
      final pendingStreams = List<_PendingStream>.from(
        st.pendingStreams.values,
      );
      for (final pst in pendingStreams) {
        if (!pst.start.isCompleted) {
          pst.start.completeError(Exception('p2p_closed'));
        }
        _failP2pStreamBody(pst.controller, Exception('p2p_closed'));
      }
      final tunnels = List<_P2pRtcWebSocketChannel>.from(st.wsTunnels.values);
      for (final ch in tunnels) {
        ch._handleRemoteClose(1001, 'p2p_closed');
      }
    }
...
class _RtcChannelState {
  ...
  int rxPackets = 0;
  int rxBytes = 0;
  int txBinaryPackets = 0;
  int txBinaryBytes = 0;
  int txTextPackets = 0;
  int txTextBytes = 0;
  int rxBinaryPackets = 0;
  int rxBinaryBytes = 0;
  int rxTextPackets = 0;
  int rxTextBytes = 0;
  int lastTxAtMs = 0;
