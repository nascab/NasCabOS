...
import '../service/video_trans_api_service.dart';

class VideoTransController extends GetxController {
  final _api = VideoTransApiService();
  ...
  Future<bool> upsert({
    ...
    if (s.isEmpty || t.isEmpty) {
      ToastUtil.show('media_tool_video_trans_required'.tr);
      return false;
    }

    final pol = nonVideoPolicy.trim().toLowerCase();
  ...
  Future<bool> remove({required int id}) async {
    final confirmed = await DialogUtil.showConfirmDialog(
      ...
      content: 'media_tool_video_trans_delete_confirm'.tr,
      confirmText: 'ok'.tr,
