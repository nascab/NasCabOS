...
import '../service/audio_trans_api_service.dart';

class AudioTransController extends GetxController {
  static const int _requiredServerVersion = 9;
  final _api = AudioTransApiService();
  ...
  Future<bool> upsert({
    ...
    if (s.isEmpty || t.isEmpty) {
      ToastUtil.show('media_tool_audio_trans_required'.tr);
      return false;
    }

    final pol = nonAudioPolicy.trim().toLowerCase();
  ...
  Future<bool> remove({required int id}) async {
    ...
    final confirmed = await DialogUtil.showConfirmDialog(
      ...
      content: 'media_tool_audio_trans_delete_confirm'.tr,
      confirmText: 'ok'.tr,
