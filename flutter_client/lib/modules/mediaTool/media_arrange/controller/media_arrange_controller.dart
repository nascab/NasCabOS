...
import '../service/media_arrange_api_service.dart';

class MediaArrangeController extends GetxController {
  final _api = MediaArrangeApiService();
  ...
  Future<bool> upsert({
    ...
    if (s.isEmpty || t.isEmpty) {
      ToastUtil.show('media_tool_arrange_required'.tr);
      return false;
    }
  ...
  Future<bool> remove({required int id}) async {
    final confirmed = await DialogUtil.showConfirmDialog(
      ...
      content: 'media_tool_arrange_delete_confirm'.tr,
      confirmText: 'ok'.tr,
