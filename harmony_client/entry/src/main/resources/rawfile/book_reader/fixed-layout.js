...
export class FixedLayout extends HTMLElement {
    ...
    #ensureDocStyles(doc) {
        if (!doc) return
        const host = doc.head ?? doc.documentElement ?? doc
        if (!host) return

        const existing = this.#styleMap.get(doc)
        if (existing) {
            const [$beforeStyle, $style] = existing
    ...
    setStyles(styles) {
        ...
        for (const iframe of this.#root.querySelectorAll('iframe')) {
            ...
            if (doc) this.#ensureDocStyles(doc)
        }
    }
