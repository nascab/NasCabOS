...
import com.nascabos.tv.modules.photo.library.presenter.PhotoLibraryCardPresenter
import com.nascabos.tv.modules.photo.timeline.PhotoTimelineBrowseFragment
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch

class PhotoLibraryGridFragment : VerticalGridSupportFragment() {
    private val kind: PhotoLibraryKind by lazy {
        val raw = arguments?.getString(ARG_KIND)?.trim().orEmpty()
        runCatching { PhotoLibraryKind.valueOf(raw) }.getOrNull() ?: PhotoLibraryKind.Album
    }

    private val viewModel: PhotoLibraryListViewModel by viewModels {
        Factory(requireActivity().application, kind)
    }

    private val adapter by lazy { ArrayObjectAdapter(PhotoLibraryCardPresenter(requireContext(), kind)) }
    ...
    private var lastState: PhotoLibraryUiState? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        parentFragmentManager.setFragmentResultListener(RESULT_KEY_SEARCH, this) { _, bundle ->
            ...
            Log.d("PhotoLibraryGrid", "result_search q='$q'")
        ...
        parentFragmentManager.setFragmentResultListener(RESULT_KEY_SORT, this) { _, bundle ->
            ...
            val field = runCatching { PhotoLibrarySortField.valueOf(fieldRaw) }.getOrNull()
            val order = runCatching { PhotoLibrarySortOrder.valueOf(orderRaw) }.getOrNull()
            if (field != null && order != null) {
                viewModel.setSort(PhotoLibrarySortState(field = field, order = order))
            }
        }

        title =
            when (kind) {
                PhotoLibraryKind.Album -> getString(R.string.home_photo_custom_albums)
                PhotoLibraryKind.SmartAlbum -> getString(R.string.home_photo_smart_albums)
                PhotoLibraryKind.Collection -> getString(R.string.home_photo_collections)
            }
        ...
        onItemViewClickedListener =
            OnItemViewClickedListener { _, item, _, _ ->
                val data = item as? PhotoLibraryListItem ?: return@OnItemViewClickedListener
    ...
    private fun renderState(state: PhotoLibraryUiState) {
    ...
    fun openOptionsFromActivity() {
        ...
        PhotoLibraryOptionsDialogFragment
            .newInstance(
                kind = kind.name,
            ...
            .show(activity.supportFragmentManager, "photo_library_options")
    }

    private fun openLibraryItem(item: PhotoLibraryListItem) {
        val activity = activity ?: return
        val aid = if (item.kind == PhotoLibraryKind.Album) item.id else null
        val cid = if (item.kind == PhotoLibraryKind.Collection) item.id else null
        val sid = if (item.kind == PhotoLibraryKind.SmartAlbum) item.id else null
        val prefix =
            when (item.kind) {
                PhotoLibraryKind.Album -> getString(R.string.photo_library_prefix_album)
                PhotoLibraryKind.SmartAlbum -> getString(R.string.photo_library_prefix_smart_album)
                PhotoLibraryKind.Collection -> getString(R.string.photo_library_prefix_collection)
            }.trim()
    ...
    private data class RenderKey(
        val keyword: String,
        val sort: PhotoLibrarySortState,
    )

    private fun itemsSignature(items: List<PhotoLibraryListItem>): Long {
    ...
    class Factory(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(PhotoLibraryListViewModel::class.java)) {
                return PhotoLibraryListViewModel(app, kind) as T
            }
    ...
    companion object {
        ...
        const val RESULT_KEY_SEARCH = "photo_library_result_search"
        const val RESULT_KEY_SORT = "photo_library_result_sort"
        ...
        fun newInstance(kind: PhotoLibraryKind): PhotoLibraryGridFragment {
            return PhotoLibraryGridFragment().apply {
