package com.nascabos.tv.modules.photo.library.presenter
...
import com.nascabos.tv.modules.photo.library.PhotoLibraryListItem

class PhotoLibraryCardPresenter(
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
        ...
        val data = item as? PhotoLibraryListItem ?: return
        ...
        val sub = if (kind == PhotoLibraryKind.SmartAlbum) "" else data.type.trim()
        subtitle.text = sub
        subtitle.visibility = if (sub.isNotEmpty()) View.VISIBLE else View.GONE

        tag.text =
            when (kind) {
                PhotoLibraryKind.Album -> context.getString(R.string.photo_library_tag_album)
                PhotoLibraryKind.SmartAlbum -> context.getString(R.string.photo_library_tag_smart_album)
                PhotoLibraryKind.Collection -> context.getString(R.string.photo_library_tag_collection)
            }
        ...
        val paths =
            data.previews.mapNotNull { it.fullPath.trim().ifEmpty { null } }.take(4)
