...
class PhotoLibraryOptionsDialogFragment : DialogFragment() {
        ...
        listView.setOnItemClickListener { _, _, position, _ ->
            when (rows.getOrNull(position)?.id) {
                ...
                ID_SORT -> {
                    ...
                    PhotoLibrarySortDialogFragment.newInstance(sortField, sortOrder).show(parentFragmentManager, "photo_library_sort")
                }
                ID_CLEAR -> {
                    parentFragmentManager.setFragmentResult(PhotoLibraryGridFragment.RESULT_KEY_SEARCH, bundleOf(PhotoLibraryGridFragment.RESULT_FIELD_QUERY to ""))
                    dismissAllowingStateLoss()
                }
        ...
        AlertDialog.Builder(ctx, R.style.Theme_NasCabTv_AlertDialog)
            ...
            .setPositiveButton(getString(R.string.video_list_action_apply)) { _, _ ->
                ...
                parentFragmentManager.setFragmentResult(
                    PhotoLibraryGridFragment.RESULT_KEY_SEARCH,
                    bundleOf(PhotoLibraryGridFragment.RESULT_FIELD_QUERY to q),
                )
                dismissAllowingStateLoss()
            }
            .setNeutralButton(getString(R.string.video_list_action_clear_search)) { _, _ ->
                parentFragmentManager.setFragmentResult(
                    PhotoLibraryGridFragment.RESULT_KEY_SEARCH,
                    bundleOf(PhotoLibraryGridFragment.RESULT_FIELD_QUERY to ""),
                )
                dismissAllowingStateLoss()
            }
    ...
    private fun sortLabel(fieldRaw: String, orderRaw: String): String {
        val field = runCatching { PhotoLibrarySortField.valueOf(fieldRaw) }.getOrNull() ?: PhotoLibrarySortField.CreateTime
        val order = runCatching { PhotoLibrarySortOrder.valueOf(orderRaw) }.getOrNull() ?: PhotoLibrarySortOrder.Desc
        return when (field) {
            PhotoLibrarySortField.CreateTime ->
                if (order == PhotoLibrarySortOrder.Desc) getString(R.string.video_library_sort_create_time_desc) else getString(R.string.video_library_sort_create_time_asc)
            PhotoLibrarySortField.Name ->
                if (order == PhotoLibrarySortOrder.Asc) getString(R.string.video_library_sort_name_asc) else getString(R.string.video_library_sort_name_desc)
        }
    }
    ...
    companion object {
        ...
        fun newInstance(
            ...
            return PhotoLibraryOptionsDialogFragment().apply {
                arguments =
                    bundleOf(
                        ARG_KIND to kind,
