...
class tableMusicCollection {
  constructor() {
    this.tableName = 'music_collection';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['uid', 'name'], name: 'idx_music_collection_uid_name_unique', unique: true },
      { columns: ['uid'], name: 'idx_music_collection_uid', unique: false },
      { columns: ['create_time'], name: 'idx_music_collection_create_time', unique: false },
    ];
...
module.exports = new tableMusicCollection();
