...
class tableVideoFavorite {
  ...
  async createTable(connection = null) {
    ...
    if (!exists) {
      ...
      Logger.info(`✅ video_favorite table created`);
    }
  }

  async createIndexes(connection = null) {
    const knex = connection && connection.knex ? connection.knex : knexUtil.getInstance(dbUtil.DB_PATHS.VIDEO_DB);
    const idxUnique = 'idx_video_fav_uid_index_unique';
    ...
    if (!uniqueExists) {
      ...
      Logger.info(`✅ video_favorite unique index created`);
    }

    const idxUid = 'idx_video_fav_uid';
    ...
    if (!uidExists) {
      ...
      Logger.info(`✅ video_favorite index (uid) created`);
    }

    const idxIndexId = 'idx_video_fav_index_id';
    ...
    if (!indexIdExists) {
      ...
      Logger.info(`✅ video_favorite index (index_id) created`);
    }

    const idxCreateTime = 'idx_video_fav_uid_create_time';
    ...
    if (!createTimeExists) {
      ...
      Logger.info(`✅ video_favorite index (uid,create_time) created`);
    }
  }
}

module.exports = new tableVideoFavorite();
