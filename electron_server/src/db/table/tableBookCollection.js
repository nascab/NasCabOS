...
class tableBookCollection {
  constructor() {
    this.tableName = 'book_collection';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['uid', 'name'], name: 'idx_book_collection_uid_name_unique', unique: true },
      { columns: ['uid'], name: 'idx_book_collection_uid', unique: false },
      { columns: ['create_time'], name: 'idx_book_collection_create_time', unique: false },
    ];
...
module.exports = new tableBookCollection();
