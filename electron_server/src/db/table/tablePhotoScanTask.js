...
class tablePhotoScanTask {
  constructor() {
    this.tableName = 'photo_scan_task';
  }
  ...
  async enqueueScanPaths(scanPaths = [], remark = '', connection = null) {
    ...
    Logger.info(`🔍 Found ${list.length} photo source(s) to scan`);
    ...
    const inserted = await knex.transaction(async trx => {
      ...
      for (const p of list) {
        if (!existing.has(p)) {
          Logger.info(`🔍 New photo source needs scan: ${p}`);
          toInsert.push({
            scan_path: p,
    ...
    if (inserted > 0) Logger.info(`✅ Added ${inserted} photo scan task(s)`);
    return inserted;
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['scan_path'], name: 'idx_photo_scan_task_scan_path', unique: false },
      { columns: ['create_time'], name: 'idx_photo_scan_task_create_time', unique: false },
    ];
...
const tablePhotoScanTaskInstance = new tablePhotoScanTask();

module.exports = tablePhotoScanTaskInstance;
