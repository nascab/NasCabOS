...
class tablePhotoAlbumIndex {
  constructor() {
    this.tableName = 'photo_album_index';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      {
        columns: ['album_id', 'file_hash'],
        name: 'idx_photo_album_index_album_hash_unique',
        unique: true,
      },
      { columns: ['album_id'], name: 'idx_photo_album_index_album_id', unique: false },
      { columns: ['is_cover'], name: 'idx_photo_album_index_is_cover', unique: false },
    ];
...
module.exports = new tablePhotoAlbumIndex();
