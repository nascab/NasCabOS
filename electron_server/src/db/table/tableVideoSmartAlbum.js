...
class tableVideoSmartAlbum {
  constructor() {
    this.tableName = 'video_smart_album';
  }
  ...
  async createTable(connection = null) {
    ...
    if (!tableExists) {
      await knex.schema.createTable(this.tableName, table => {
        ...
        table.string('type').notNullable().defaultTo('condition');
        table.text('filter_content').notNullable().defaultTo('{}');
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      {
        columns: ['uid', 'name'],
        name: 'idx_video_smart_album_uid_name_unique',
        unique: true,
      },
      { columns: ['uid'], name: 'idx_video_smart_album_uid', unique: false },
      { columns: ['create_time'], name: 'idx_video_smart_album_create_time', unique: false },
      { columns: ['type'], name: 'idx_video_smart_album_type', unique: false },
    ];
...
module.exports = new tableVideoSmartAlbum();
