...
class tableBookScanTask {
  constructor() {
    this.tableName = 'book_scan_task';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['scan_path'], name: 'idx_book_scan_task_scan_path', unique: false },
      { columns: ['create_time'], name: 'idx_book_scan_task_create_time', unique: false },
    ];
  ...
  async enqueueScanPaths(scanPaths = [], remark = '', connection = null) {
    ...
    if (inserted > 0) Logger.info(`✅ Added ${inserted} book scan task(s)`);
    return inserted;
  }
}

module.exports = new tableBookScanTask();
