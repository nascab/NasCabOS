...
class tableVideoAlbum {
  constructor() {
    this.tableName = 'video_album';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['uid', 'name'], name: 'idx_video_album_uid_name_unique', unique: true },
      { columns: ['uid'], name: 'idx_video_album_uid', unique: false },
      { columns: ['create_time'], name: 'idx_video_album_create_time', unique: false },
    ];
...
module.exports = new tableVideoAlbum();
