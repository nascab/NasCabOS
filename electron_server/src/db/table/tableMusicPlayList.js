...
class tablePlayList {
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['uid', 'name'], name: 'uidx_play_list_uid_name', unique: true },
      { columns: ['uid'], name: 'idx_play_list_uid', unique: false },
      { columns: ['create_time'], name: 'idx_play_list_create_time', unique: false },
    ];
  ...
  async createTriggers(connection = null) {
    ...
    const hasMapTable = await knex.schema.hasTable('play_list2index').catch(() => false);
    if (!hasMapTable) return;

    const triggers = [
      {
        name: 'trg_play_list_delete_play_list2index',
        sql: `
          CREATE TRIGGER trg_play_list_delete_play_list2index
          AFTER DELETE ON play_list
          FOR EACH ROW
          BEGIN
            DELETE FROM play_list2index WHERE list_id = OLD.id;
          END;
        `,
      },
    ];
...
module.exports = new tablePlayList();
