...
class tableVideoCollection {
  constructor() {
    this.tableName = 'video_collection';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['uid', 'name'], name: 'idx_video_collection_uid_name_unique', unique: true },
      { columns: ['uid'], name: 'idx_video_collection_uid', unique: false },
      { columns: ['create_time'], name: 'idx_video_collection_create_time', unique: false },
    ];
...
module.exports = new tableVideoCollection();
