...
class tableVideoIndex2Key {
  constructor() {
    this.tableName = 'video_index2key';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['index_id', 'key', 'key_type'], name: 'uidx_video_index2key_index_id_key_key_type', unique: true },
      { columns: ['index_id'], name: 'idx_video_index2key_index_id', unique: false },
      { columns: ['key_type', 'key'], name: 'idx_video_index2key_key_type_key', unique: false },
    ];
...
module.exports = new tableVideoIndex2Key();
