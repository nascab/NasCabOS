...
class tableVideoScanTask {
  constructor() {
    this.tableName = 'video_scan_task';
  }
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [
      { columns: ['scan_path'], name: 'idx_video_scan_task_scan_path', unique: false },
      { columns: ['create_time'], name: 'idx_video_scan_task_create_time', unique: false },
    ];
  ...
  async enqueueScanPaths(scanPaths = [], remark = '', connection = null) {
    ...
    if (inserted > 0) Logger.info(`✅ Added ${inserted} video scan task(s)`);
    return inserted;
  }
}

module.exports = new tableVideoScanTask();
