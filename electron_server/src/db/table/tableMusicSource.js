...
class tableMusicSource {
  ...
  async createTable(connection = null) {
    ...
    if (!tableExists) {
      await knex.schema.createTable(this.tableName, table => {
        ...
        table.string('show_type'); //以文件为单元展示 或以文件夹为单元展示 music series
      });
  ...
  async getScanWhenStartPaths(connection = null) {
    ...
    const rows = await knex(this.tableName)
      ...
      .catch(err => {
        Logger.error('❌ music source query failed:', err);
        return [];
      });
  ...
  async createIndexes(connection = null) {
    ...
    const targetIndexes = [{ columns: ['path'], name: 'uidx_music_source_path', unique: true }];
...
module.exports = new tableMusicSource();
