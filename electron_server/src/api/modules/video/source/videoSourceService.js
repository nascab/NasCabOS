...
class VideoSourceService {
  ...
  async addSource(input = {}) {
    ...
    const mediaType = this.normalizeMediaType(input.media_type ?? input.mediaType);
    if (!mediaType) {
      throw new Error('validation.VALIDATION_ERROR');
    }
    const matchNfo = this.normalizeMatchNfo(input.match_nfo ?? input.matchNfo);

    const rows = await this.knex(this.tableName).select('id', 'path').orderBy('id', 'asc');

    const exact = rows.find(r => r.path === normalizedPath);
    if (exact) {
      throw new Error('video.VIDEO_SOURCE_EXISTS');
    }

    const parentRow = rows.find(r => this.isParentPath(r.path, normalizedPath));
    if (parentRow) {
      const err = new Error('video.VIDEO_SOURCE_PARENT_EXISTS');
  ...
  async updateMediaType(id, mediaType) {
    const sourceId = Number(id);
    if (!Number.isFinite(sourceId) || sourceId <= 0) {
      throw new Error('validation.VALIDATION_ERROR');
    }
    const mt = this.normalizeMediaType(mediaType);
    if (!mt) throw new Error('validation.VALIDATION_ERROR');

    return this.knex.transaction(async trx => {
      const source = await trx(this.tableName).where({ id: sourceId }).first('id', 'path', 'media_type');
      if (!source || !source.id) throw new Error('common.NOT_FOUND');

      const resolved = source.path ? String(source.path) : '';
      if (resolved) {
        const scanPrefix = resolved.endsWith(path.sep) ? resolved : `${resolved}${path.sep}`;
        await trx('video_index')
          .where(qb => {
      ...
      await trx(this.tableName).where({ id: sourceId }).update({ media_type: mt });
      return trx(this.tableName).where({ id: sourceId }).first();
    });
  }

  async updateMatchNfo(id, matchNfo) {
    const sourceId = Number(id);
    if (!Number.isFinite(sourceId) || sourceId <= 0) {
      throw new Error('validation.VALIDATION_ERROR');
    }
    const v = this.normalizeMatchNfo(matchNfo);

    const affected = await this.knex(this.tableName).where({ id: sourceId }).update({ match_nfo: v });
    if (!affected) throw new Error('common.NOT_FOUND');
    return this.knex(this.tableName).where({ id: sourceId }).first();
  }
  ...
  async relocateSource(id, newPath) {
    ...
    return this.knex.transaction(async trx => {
      ...
      const updatedDirect = await trx('video_index')
        .where({ path: oldPath })
        .update({ path: normalizedNewPath })
        .catch(() => 0);

      const startAt = oldPrefix.length + 1;
      const updatedSubtree = await trx('video_index')
      ...
      const updatedRootDirIndex = await trx('video_index')
