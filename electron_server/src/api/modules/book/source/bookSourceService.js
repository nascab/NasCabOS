...
class BookSourceService {
  ...
  async listSources() {
    ...
    return (rows || []).map(r => {
      ...
      return { ...r, show_type: showType || 'book' };
    });
  }
  ...
  normalizeShowType(input) {
    const v = String(input || '')
      .trim()
      .toLowerCase();
    if (!v) return '';
    if (v === 'book' || v === 'series') return v;
    return '';
  }
  ...
  async addSource(inputPath) {
    ...
    if (exact) {
      throw new Error('book.BOOK_SOURCE_EXISTS');
    }

    const parentRow = rows.find(r => this.isParentPath(r.path, normalizedPath));
    if (parentRow) {
      const err = new Error('book.BOOK_SOURCE_PARENT_EXISTS');
  ...
  async updateSource(id, payload = {}) {
    ...
    if (payload.show_type !== undefined) {
      const st = this.normalizeShowType(payload.show_type);
      if (!st) throw new Error('validation.VALIDATION_ERROR');
      data.show_type = st;
    }
    ...
    const result = await this.knex.transaction(async trx => {
      ...
      const oldShowType = current && current.show_type ? String(current.show_type) : 'book';
      ...
      if (showTypeChanged && resolved) {
        ...
        const existedTask = await trx('book_scan_task')
          .where({ scan_path: resolved })
          .first('id')
          .catch(() => null);
  ...
  async relocateSource(id, newPath) {
    ...
    return this.knex.transaction(async trx => {
      ...
      const updatedDirect = await trx('book_index')
        .where({ path: oldPath })
        .update({ path: normalizedNewPath })
        .catch(() => 0);

      const startAt = oldPrefix.length + 1;
      const updatedSubtree = await trx('book_index')
      ...
      const updatedRootDirIndex = await trx('book_index')
...
module.exports = BookSourceService;
