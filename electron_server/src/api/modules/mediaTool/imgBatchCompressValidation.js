...
const ImgBatchCompressValidation = {
  validateList() {
  ...
  validateUpsert() {
    return [
      ...
      body('out_format').notEmpty().isIn(['jpeg', 'png', 'webp']).withMessage('validation.VALIDATION_ERROR'),
      body('quality').notEmpty().isInt({ min: 30, max: 100 }).withMessage('validation.VALIDATION_ERROR'),
      body('out_size').optional({ nullable: true }).isInt({ min: 10, max: 20000 }).withMessage('validation.VALIDATION_ERROR'),
      body('non_image_policy').notEmpty().isIn(['skip', 'copy']).withMessage('validation.VALIDATION_ERROR'),
    ];
  },
...
module.exports = ImgBatchCompressValidation;
