...
const { ImgBatchCompressService } = require('./imgBatchCompressService');
...
class ImgBatchCompressController {
  async _startByIpc({ id }) {
    ...
    const requestId = `startImgBatchCompressTask_${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const wait = waitForIpcResponse({ requestId, responseType: 'startImgBatchCompressTaskResponse', timeoutMs: 20000 });
    process.send({ type: 'startImgBatchCompressTask', data: { requestId, id: idNum }, timestamp: Date.now() });
  ...
  async _stopByIpc({ id }) {
    ...
    if (!Number.isFinite(idNum) || idNum <= 0) throw buildHttpError('common.INVALID_PARAMS', 400);
    if (typeof process.send !== 'function') throw buildHttpError('common.ERROR', 500);
    const requestId = `stopImgBatchCompressTask_${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const wait = waitForIpcResponse({ requestId, responseType: 'stopImgBatchCompressTaskResponse', timeoutMs: 20000 });
    process.send({ type: 'stopImgBatchCompressTask', data: { requestId, id: idNum }, timestamp: Date.now() });
    const data = await wait;
    if (!data.stopped) {
      const mapped = mapStartStopErrorToResponse(data && data.error ? data.error : '');
      throw buildHttpError(mapped.msgKey, mapped.statusCode);
  ...
  async list(req, res) {
    try {
      const service = new ImgBatchCompressService(req.dbMain);
      ...
      Logger.error('imgBatchCompress list failed', e);
  ...
  async upsert(req, res) {
    try {
      const service = new ImgBatchCompressService(req.dbMain);
      const { id, source_path, target_path, out_format, quality, out_size, non_image_policy } = req.body || {};
      const data = await service.upsert({
        ...
        quality,
        outSize: out_size,
        nonImagePolicy: non_image_policy,
      });
      return ResponseUtil.success(req, res, data, 'common.SUCCESS', 200);
    } catch (e) {
      Logger.error('imgBatchCompress upsert failed', e);
  ...
  async remove(req, res) {
    try {
      ...
      Logger.error('imgBatchCompress delete failed', e);
      const msgKey = e && e.message ? String(e.message) : 'common.ERROR';
      const statusCode = e && e.statusCode ? Number(e.statusCode) : 500;
      return ResponseUtil.error(req, res, msgKey, statusCode);
    }
  ...
  async start(req, res) {
    try {
      ...
      Logger.error('imgBatchCompress start failed', e);
      const msgKey = e && e.message ? String(e.message) : 'common.ERROR';
      const statusCode = e && e.statusCode ? Number(e.statusCode) : 500;
      return ResponseUtil.error(req, res, msgKey, statusCode);
    }
  ...
  async stop(req, res) {
    try {
      ...
      Logger.error('imgBatchCompress stop failed', e);
      const msgKey = e && e.message ? String(e.message) : 'common.ERROR';
      const statusCode = e && e.statusCode ? Number(e.statusCode) : 500;
      return ResponseUtil.error(req, res, msgKey, statusCode);
    }
...
module.exports = new ImgBatchCompressController();
